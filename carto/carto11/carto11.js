...
const promise3 = d3.json("france.json");
...
Promise.all([promise1, promise2, promise3]).then(function(carto){
	...
	const featureCollectionFr = topojson.feature(carto[2], carto[2].objects.france);
	...
	svg
		.call(d3.zoom()
			.on("zoom", function(){
				...
				ctrSquare
					.attr("x", (d)=>{return path.centroid(d)[0] - 7/d3.event.transform.k/2;})
					.attr("y", (d)=>{return path.centroid(d)[1] - 7/d3.event.transform.k/2;})
